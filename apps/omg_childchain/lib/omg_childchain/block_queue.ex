...
defmodule OMG.ChildChain.BlockQueue do
  ...
  defmodule Server do
    ...
    def handle_continue(:setup, %{}) do
      ...
      {:ok, finality_threshold} = Application.fetch_env(:omg_childchain, :submission_finality_margin)
      ...
      {:ok, state} =
        with {:ok, _state} = result <-
               Core.new(
                 ...
                 minimal_enqueue_block_gap: Application.fetch_env!(:omg_childchain, :child_block_minimal_enqueue_gap),
      ...
      interval = Application.fetch_env!(:omg_childchain, :block_queue_eth_height_check_interval_ms)
