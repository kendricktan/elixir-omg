...
defmodule OMG.ChildChain.Integration.Fixtures do
  ...
  deffixture fee_file(token) do
    ...
    default_path = Application.fetch_env!(:omg_childchain, :fee_specs_file_path)
    Application.put_env(:omg_childchain, :fee_specs_file_path, path, persistent: true)

    on_exit(fn ->
      Application.put_env(:omg_childchain, :fee_specs_file_path, default_path)
    end)
  ...
  deffixture omg_child_chain(root_chain_contract_config, fee_file, db_initialized) do
    ...
    {:ok, started_apps} = Application.ensure_all_started(:omg_childchain)
